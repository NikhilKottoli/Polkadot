// Gas Estimation Service - Solidity Only
// Extracted from gasEstimation.js and cleaned up

import { solidityCompiler, getCompilerVersions } from "@agnostico/browser-solidity-compiler";

// Simple Solidity Compiler Class
...
// Generate mock bytecode for estimation
...
// Generate gas estimates from ABI
...
const getComplexityScore = (funcName, code) => {
...
const estimateContractGas = async (solidityCode, contractName) => {
  try {
    ...
    return {
      ...
      error: error.message,
      fallback: {
        deploymentGas: { estimated: 100000, method: 'fallback' },
        functionGasEstimates: {},
        totalEstimatedCost: { total: 100000 }
      }
    };
  }
};
...
const identifyHighGasFunctions = (gasEstimation) => {
  if (!gasEstimation.success || !gasEstimation.functionGasEstimates) {
    return [];
  }
  
  const highGasThreshold = 100000; // 100k gas threshold
  const highGasFunctions = [];
  
  Object.entries(gasEstimation.functionGasEstimates).forEach(([functionName, data]) => {
    if (data.estimated > highGasThreshold) {
      highGasFunctions.push({
        name: functionName,
        gasEstimate: data.estimated,
        complexity: data.complexity || 'unknown',
        type: data.type || 'nonpayable'
      });
    }
  });
  
  console.log(`🔍 [GasEstimation] Found ${highGasFunctions.length} high-gas functions (>${highGasThreshold} gas)`);
...
export const GasEstimationService = {
  estimateContractGas,
  identifyHighGasFunctions,
  compileSolidityRobust,
